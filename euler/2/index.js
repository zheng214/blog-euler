...
module.exports = {
   ...
   * @question What is the greatest product of four adjacent numbers in the same direction (up, down, left, right, diagonal) in this [20×20 grid @asset p11_data.txt]?
   * @guide
   ...
   * We rely on the fact that D(a * b) = D(a) * D(b) for any integers a, b.
   ...
   * 2. D(t) = D(n) * D((n+1)/2) if n is odd
   * 
   * One by one, We check each triangular number for its number of divisors.
   * 
   ...
   * @guide
   * We iteratively multiply our result by 2 and we store each digit inside an array (the array is "reversed": 1024 is stored as [4,2,0,1])
   * 
   * After 1000 iterations we add the digits of our array.
   */
  e16() {
    let resultArr = [2];
    let sumOfDigits = 2;
   ...
   * That is, 3 + 7 + 4 + 9 = 23.
   *
   * @question Find the maximum total from top to bottom of [this triangle @asset p18_triangle.txt].
   * @guide
   * We iterate through each row, and for each number in each row, we keep track of the maximum total from the top to that number, by using this update rule:
   * 
   * Each number has two numbers above it, named "left parent" and "right parent". For each number, we keep the result of the parent with the highest "path value".
   */
  e18() {
    const data18 = `
    75
   ...
   * @guide
   * We just loop through each month of each year, and calculate that day's distance to 1 Jan 1900, which is known to be a Monday.
   * From that distance we can tell if the day is a sunday or not, by computing its remainder when dividing by 7.
   */
  e19() {
